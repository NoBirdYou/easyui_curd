...
function initAdd() {
     ...
     */
    $(function(){
        $('#computer_extend_add_list').datagrid({
            title:'扩展设备',
            iconCls:'icon-edit',
            fit:true,
            rownumbers:true,
...
function updateActions(index){
    $('#computer_extend_add_list').datagrid('updateRow',{
        index: index,
        row:{}
    });
}
...
function editrow(target){
    $('#computer_extend_add_list').datagrid('beginEdit', getRowIndex(target));
}
...
function deleterow(target){
    $.messager.confirm('Confirm','Are you sure?',function(r){
        if (r){
            $('#computer_extend_add_list').datagrid('deleteRow', getRowIndex(target));
        }
    });
}
...
function saverow(target){
    $('#computer_extend_add_list').datagrid('endEdit', getRowIndex(target));
}
...
function cancelrow(target){
    $('#computer_extend_add_list').datagrid('cancelEdit', getRowIndex(target));
}
 ...
 */
function insert(){
    var row = $('#computer_extend_add_list').datagrid('getSelected');
    if (row){
        var index = $('#computer_extend_add_list').datagrid('getRowIndex', row);
    } else {
        index = 0;
    }
    $('#computer_extend_add_list').datagrid('insertRow', {
        index: index,
        row:{

        }
    });
    $('#computer_extend_add_list').datagrid('selectRow',index);
    $('#computer_extend_add_list').datagrid('beginEdit',index);
}
...
function submitData(){
    ...
    mainInfoJsonData=$('#addMainInfo').serializeObject();
    mainInfoJsonData["extendDevices"]= $('#computer_extend_add_list').datagrid('getRows');
    delete mainInfoJsonData['editing'];
    ajaxLoading();
    $.ajax({
        url:"/computer/addData.do",
